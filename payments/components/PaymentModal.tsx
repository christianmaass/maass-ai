/**
 * PAYMENT MODAL COMPONENT (moved to payments domain)
 */
...
const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, selectedPlan }) => {
  ...
  const handlePayment = async () => {
    try {
      ...
      if (!accessToken) {
        console.error('Failed to get access token for payment');
      ...
      if (selectedPlan?.name === 'Free') {
        ...
        if (response.ok) {
          ...
          throw new Error(
            `Fehler beim Aktivieren des Free-Plans: ${errorData.error || response.status}`,
          );
        }
        return;
      }
      ...
      if (selectedPlan?.name === 'Plus') {
        ...
        alert(
          `Für den ${selectedPlan?.displayName}-Plan kontaktieren Sie uns bitte unter: christian@christianmaass.com`,
        );
        onClose();
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative text-gray-800">
        ...
        <h1 className="text-2xl font-bold mb-6 text-center">{selectedPlan.displayName} Tarif</h1>

        {requiresLogin ? (
          <div className="text-center space-y-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-yellow-800">Sie müssen sich zuerst anmelden, um einen Tarif zu wählen.</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-lg p-4 text-center">
              ...
              <div className="text-2xl font-bold text-blue-600 mb-2">{selectedPlan.price}</div>
              {selectedPlan.name === 'Free' && (
                <p className="text-sm text-gray-600">5 Cases pro Woche • Grundlegende Features</p>
              )}
              {selectedPlan.name === 'Plus' && (
                <p className="text-sm text-gray-600">25 Cases pro Woche • Erweiterte Features</p>
              )}
              {(selectedPlan.name === 'Business' || selectedPlan.name === 'Bildungsträger') && (
                <p className="text-sm text-gray-600">Individuelle Lösung • Persönliche Beratung</p>
              )}
            </div>
            ...
            {selectedPlan.name === 'Plus' && (
              <p className="text-xs text-gray-500 text-center">Sichere Zahlung über Stripe • Monatlich kündbar</p>
            )}
          </div>
        )}
      </div>
