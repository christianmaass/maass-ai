 ...
 * - Secure case management with proper authentication
...
const CasesList: React.FC<CasesListProps> = ({ onCaseSelect, currentCaseId, className = '' }) => {
  // =============================================================================
  ...
  // moved below loadCaseHistory to avoid TDZ
  ...
  const getMockCaseHistory = (): CaseHistoryItem[] => [
    {
      ...
      case_data: {
        ...
        description:
          'Ein mittelständisches Unternehmen möchte in den E-Commerce-Markt einsteigen...',
        case_type: {
    ...
    },
    {
      ...
      case_data: {
        ...
        description:
          'Ein Automobilzulieferer steht unter Kostendruck und muss seine Produktionskosten senken...',
        case_type: {
          name: 'Operations',
          difficulty_level: 2,
        },
  ...
  const loadCaseHistory = useCallback(async () => {
    try {
      ...
      if (!isAuthenticated() || !user) {
        console.error('User not authenticated for case history');
      ...
      if (!accessToken) {
        console.error('Failed to get access token for case history');
        throw new Error('Authentifizierungsfehler. Bitte erneut einloggen.');
      }

      // Use navaa Guidelines - API call instead of direct DB access
      const response = await fetch('/api/cases/user-case-history', {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to load case history');
      }

      const { cases: responses } = await response.json();

      if (!responses) {
        console.error('No case history data received');
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">Deine Cases</h2>
            <span className="text-sm text-gray-500">{caseHistory.length} Cases insgesamt</span>
          </div>
        </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-col items-end space-y-2 ml-4">
                      ...
                      {/* Difficulty Badge */}
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getDifficultyColor(item.case_data.case_type.difficulty_level)}`}
                      >
