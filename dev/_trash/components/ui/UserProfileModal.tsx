 ...
 * - Secure user profile operations with proper authentication
 * - No direct supabase.auth calls
 *
 * @version 2.0.0 (WP-B1 Migration - FINAL)
 */
...
const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose }) => {
  // =============================================================================
  // NAVAA AUTH INTEGRATION (WP-B1 Migration - FINAL COMPONENT)
  // =============================================================================

  const { user: authUser, getAccessToken, isAuthenticated } = useAuth();

  const [user, setUser] = useState<any>(null);
  const [userProfile, setUserProfile] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  // moved below fetchUserData to avoid TDZ
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      if (!isAuthenticated() || !authUser) {
        console.error('User not authenticated for profile modal');
        setLoading(false);
        return;
      }

      // Use user from AuthContext (no direct supabase.auth call needed)
      setUser(authUser);

      // Get JWT token for API call (MANDATORY per navaa Guidelines)
      const accessToken = await getAccessToken();
      if (!accessToken) {
        console.error('Failed to get access token for user profile');
        setLoading(false);
        return;
      }

      // Get user tariff info from API with JWT token
  ...
  }, [authUser, getAccessToken, isAuthenticated]);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-2">
                <p>
                  <strong>E-Mail:</strong> {user?.email || 'Nicht verfügbar'}
                </p>
                <p>
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span>
                      <strong>Cases diese Woche:</strong>
                    </span>
                    <span>
                      {userProfile?.casesUsed || 0} / {userProfile?.casesLimit || 5}
                    </span>
                  </div>
                ...
                {userProfile?.casesLimitMonth &&
                  userProfile.casesLimitMonth !== userProfile.casesLimit * 4 && (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span>
                          <strong>Cases diesen Monat:</strong>
                        </span>
                        <span>
                          {userProfile?.casesUsedMonth || 0} / {userProfile?.casesLimitMonth || 20}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                ...
                {userProfile?.currentPeriodEnd && (
                  <div className="text-sm text-gray-600">
                    ...
                    {new Date(userProfile.currentPeriodEnd).toLocaleDateString('de-DE')}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-3">
                {userProfile?.tariff === 'Free' ? (
                  ...
                  <div>
                    <p className="text-gray-600 mb-3">
                      Du kannst dein Abonnement jederzeit kündigen. Die Kündigung wird zum Ende des
