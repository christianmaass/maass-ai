...
const ResponseInput: React.FC<ResponseInputProps> = ({ caseData, onResponseSubmitted }) => {
  ...
  const { user, getAccessToken, error: authError } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check authentication using navaa Guidelines
    if (!user) {
      setError('Please log in to submit response');
      return;
    }
    ...
    try {
      ...
      // Submit response to API with JWT token (navaa Guidelines)
    ...
    return (
      <div className="w-full bg-white rounded-xl shadow p-8 text-center">
        ...
        <p className="text-gray-500">
          Generiere zuerst einen Case, um deine Antwort eingeben zu können.
        </p>
      </div>
    );
  }
...
4. Empfehlung: ..."
          ...
          <div className="flex justify-between items-center mt-2">
            <span className="text-sm text-gray-500">{response.length} Zeichen</span>
            <span className="text-sm text-gray-500">💡 Tipp: Nutze MECE-Strukturierung</span>
          </div>
        </div>
